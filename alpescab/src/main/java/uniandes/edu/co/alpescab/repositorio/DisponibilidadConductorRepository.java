...
public interface DisponibilidadConductorRepository extends JpaRepository<DISPONIBILIDAD_CONDUCTOR, Integer> {
    ...
    @Query("SELECT d FROM DISPONIBILIDAD_CONDUCTOR d WHERE d.idUsuarioConductor.ID_PERSONA = :id_usuario_conductor AND d.horario_inicio < :horario_fin AND d.horario_fin > :horario_inicio")
    ...
    @Query("SELECT d FROM DISPONIBILIDAD_CONDUCTOR d WHERE d.idUsuarioConductor.ID_PERSONA = :id_usuario_conductor AND d.id_disponibilidad != :id_disponibilidad AND d.horario_inicio < :horario_fin AND d.horario_fin > :horario_inicio")
